...
export function CharacterPanel({
  ...
  const { game, artifactDetail } = t.ui
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-[#241919] pt-3">
              ...
              {character.inventory.map((artifact) => (
                <button
                  key={artifact.id}
                  type="button"
                  title={artifactDetail.inspectHint}
                  onClick={() => setInspectArtifact(artifact)}
                  className="border border-[#2b2320] bg-black/40 px-2.5 py-1 text-[11px] text-[#d8c9be] transition hover:border-[#5c3a2a] hover:bg-[#1a1010] sm:text-[12px]"
                >
                  {artifact.name}
                </button>
              ))}
